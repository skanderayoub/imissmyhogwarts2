...
async function fetchCharacterLoreData() {
  try {
    const response = await fetch("https://hp-api.onrender.com/api/characters");
    if (!response.ok) throw new Error("Network response was not ok");
    const characterLoreData = await response.json();
    return characterLoreData.slice(0, 20); // Limit to 20 characters to avoid overwhelming the UI
  } catch (error) {
...
function createFloatingCandle(x, y) {
...
async function initialize() {
  await Promise.all([
    ...
    fetchMusicData(),
    fetchCursorData(),
    fetchCharacterLoreData().then((characterLoreData) => {
      const characterLoreList = document.getElementById("characterLoreList");
      characterLoreData.forEach((character) => {
        ...
        details.className = "text-sm text-yellow-200 mt-2";
        details.textContent = `House: ${character.house || "Unknown"}, Species: ${character.species || "Unknown"}`;
